    ...
    return (
        <div className="min-h-screen overflow-hidden flex md:block justify-center items-center bg-[url('/login/login-bg.jpg')] bg-cover bg-right-top md:bg-none text-greenr">
            ...
            <div className="block md:grid md:grid-cols-2 md:gap-10">
                <div className="hidden relative col-span-1 h-screen bg-[url('/login/login-bg.jpg')] bg-cover font-semibold text-white text-[100px] md:flex items-end pl-8 pb-10 leading-[110px]">
                        ...
                        <Form>
                            <div>
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <Field type="email" name="email" id="email" className="border border-greenr rounded-md pl-3 md:h-12 h-8 text-xs text-greenr z-10" />
                                    <ErrorMessage name="email" component="div" className="text-red-600 text-sm mt-1" />
                                </div>
                                <div className="flex flex-col gap-1 mt-3 md:mt-6">
                                    <label htmlFor="password" className="font-semibold text-sm">Password</label>
                                    <Field type="password" name="password" id="password" className="border border-greenr rounded-md pl-3 md:h-12 h-8 text-xs z-10" />
                                    <ErrorMessage name="password" component="div" className="text-red-600 text-sm mt-1" />
                                    <button className="text-end font-semibold mt-1 text-xs md:text-sm">Forgot password?</button>
                                </div>
                            </div>
                            <Buttons value="Sign in" className="w-full font-semibold border !important border-white md:border-none rounded-md text-sm md:text-xl mt-8 md:mt-5" type="submit" disabled={isLoading} />
                        </Form>
                    </Formik>

                    <p className="font-semibold text-xs md:text-base text-center mt-[3px] md:mt-2">or</p>
                    <button onClick={handleSubmitGoogle} className="w-full font-semibold rounded-md text-sm md:text-lg mt-[3px] md:mt-2 border border-greenr flex items-center justify-center gap-2 p-2">
