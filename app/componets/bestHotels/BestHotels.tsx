...
const BestDeals = () => {
  const { data: randomRooms, isLoading, error } = useGet10RandomAvailableRooms();
      ...
      <div className={"px-5 sm:px-10 md:px-20 lg:px-[80px]"}>
        <AnimationWrapper y={40} transition={{ ease: "easeOut", duration: 1 }}>
          <div className="mt-16 bg-[url('/homepage/besthotel.avif')] bg-cover h-[600px] rounded-2xl flex mb-20">
            <div className={"bg-gradient-to-t from-slate-800 to-transparent w-full h-full flex flex-col  md:pl-10 justify-center rounded-b-2xl"}>
              <div className={"pl-5 flex items-center justify-between pr-5 md:pr-10 mb-4"}>
                <h2 className={" text-2xl md:text-3xl  text-white font-semibold"}>
                  Best Deals For You
                </h2>
                <Buttons
                    value={"See all deals"}
                    className={"bg-white min-w-fit h-fit !text-xs md:!text-base !text-green-800 hover:text-greensecondary"}
                />
              </div>
                ...
                <CarouselContent>
                  {randomRooms?.map((room, index) => (
                      <CarouselItem
                        ...
                        <Image
                            src={room.properties.propertyPictures[0]?.imgUrl || '/placeholder.jpg'}
                            alt={room.name}
                            width={200}
                            height={115}
                        ...
                        <div className={"mt-3"}>
                          ...
                          <div className={"mt-5 flex gap-2 items-center mb-4"}>
                            <div className={"bg-green-600 px-2 flex text-white font-semibold rounded-lg py-[3px] items-center text-center"}>
                              <p className={"text-sm font-semibold"}>
                                {room.properties.averageRating.toFixed(1)}
                              </p>
                            </div>
                            <p className={"font-semibold"}>
                              {getRateLabel(room.properties.averageRating.toString())}
                            </p>
